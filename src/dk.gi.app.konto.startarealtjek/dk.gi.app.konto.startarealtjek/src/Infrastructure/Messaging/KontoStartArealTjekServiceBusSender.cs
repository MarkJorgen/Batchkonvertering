...
{
    public class KontoStartArealTjekServiceBusSender
    {
        private readonly KontoStartArealTjekSettings _settings;
        private readonly IJobLogger _logger;

        public KontoStartArealTjekServiceBusSender(KontoStartArealTjekSettings settings, IJobLogger logger)
        {
            _settings = settings;
            _logger = logger;
        }

        public virtual bool Send(KontoStartArealTjekCandidate candidate, ResolvedServiceBusSettings resolvedServiceBusSettings, int scheduleDelaySeconds)
        {
            ...
            using (var request = new HttpRequestMessage(HttpMethod.Post, endpoint))
            {
                ...
                {
                    _logger.Info("Publicerede Service Bus-job for konto " + candidate.AccountNumber + " (" + candidate.AccountId + ") via " + effective.Source + ".");
                    return true;
                }

                string body = response.Content != null ? response.Content.ReadAsStringAsync().GetAwaiter().GetResult() : string.Empty;
                _logger.Error("Service Bus-publicering fejlede for konto " + candidate.AccountNumber + ". Status=" + (int)response.StatusCode + " Reason=" + response.ReasonPhrase + ". Kilde=" + effective.Source + ". Body=" + body);
                return false;
            }
        }

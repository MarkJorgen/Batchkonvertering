...
{
    public sealed class LassoXOphoerSettings
    {
        ...
        public static LassoXOphoerSettings Create(JobConfiguration configuration)
        {
            string mode = (configuration.Get("Mode", "DRYRUN") ?? "DRYRUN").Trim().ToUpperInvariant();
            string crmConnectionTemplate = CrmScalarSettingNormalizer.Normalize(configuration.Get("CrmConnectionTemplate", string.Empty));
            string crmServerName = CrmScalarSettingNormalizer.Normalize(configuration.Get("CrmServerName", string.Empty));
            string crmClientId = CrmScalarSettingNormalizer.Normalize(configuration.Get("CrmClientId", string.Empty));
            string crmClientSecret = CrmScalarSettingNormalizer.Normalize(configuration.Get("CrmClientSecret", string.Empty));
            string crmAuthority = CrmScalarSettingNormalizer.Normalize(configuration.Get("CrmAuthority", string.Empty));
            string crmAuthorityMode = CrmScalarSettingNormalizer.Normalize(configuration.Get("CrmAuthorityMode", "AsConfigured"));

            bool verifyCrmOnly = mode == "VERIFYCRM";
            bool dryRun = mode == "DRYRUN" || mode == "SIMULATE";
            bool runMode = mode == "RUN";

            return new LassoXOphoerSettings(
                mode,
                configuration.GetBool("EnableLocalDebugLogging"),
                configuration.Get("LocalDebugLogPath", string.Empty),
                configuration.Get("MutexName", "dk.gi.app.contact.lassox.ophoer"),

...
{
    public static class ContactRegistreringOptaellingSettingsValidator
    {
        public static void Validate(ContactRegistreringOptaellingSettings settings)
        {
            if (settings == null)
            {
            ...
            {
                throw new ConfigurationErrorsException("SecondsToSleep må ikke være negativ.");
            }

            if (settings.MaxWaitCount < 0)
            {
                throw new ConfigurationErrorsException("MaxWaitCount må ikke være negativ.");
            }

            if (settings.EnableLocalDebugLogging && string.IsNullOrWhiteSpace(settings.LocalDebugLogPath))
            {
                missing.Add("LocalDebugLogPath");
            }
            ...
            if (missing.Count > 0)
            {
                throw new ConfigurationErrorsException(
                    "Følgende settings mangler til valgt mode: " + string.Join(", ", missing));
            }
        }
    }
}

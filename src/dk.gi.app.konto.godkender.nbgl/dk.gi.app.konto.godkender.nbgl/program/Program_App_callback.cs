...
namespace dk.gi.crm.app.konto.godkender.nbgl
{
    ...
    public partial class GIConsoleApp
    {
        ...
        private AppStatus CallBackFunction()
        {
            ...
            //if (appConfig.Mode.ToUpper() == "KONTOGODKENDER")
            //{
            result.SetStatus = AppStatus.StateCode.OK;

            TilfoejCVROgGodkendereRequest req = new TilfoejCVROgGodkendereRequest(crmcontext);
            TilfoejCVROgGodkendereResponse resp = req.Execute<TilfoejCVROgGodkendereResponse>();

            if (resp.Status.IsOK() != true)
                result.SetStatus = AppStatus.StateCode.AppUventetFejlIProgramKode; // Din fejlkode som retuneres til OS -  lad den stå medmindre du ønsker et andet tal (bruges nedenfor til fejlhåndtering)
            //}

...
namespace dk.gi.app.konto.opgoerelse.rykker
{
    ...
    public partial class GIConsoleApp
    {
        ...
        internal void RunOrWaitForGoSignal(Action callback, string uniqueMutexName, bool IfTimeoutRunAnyway)
        {
            ...
            catch (Exception ex)
            {
                if (ex.InnerException != null)
                    Trace.LogError(ex, "Mutex: Der er sket en Exception" + ex.InnerException.Message);
                Trace.LogError(ex, "Mutex: Der er sket en Exception" + ex.Message);
            }
            finally
            {
                ...
                if (gotMutex == false)
                {
                    ...
                    if (IfTimeoutRunAnyway == true)
                    {
                        ...
                        appStatus.SetStatus = AppStatus.StateCode.AppUventetFejlIProgramKode;
                }
            }
            ...
            {
                Trace.LogError($"RunOrWaitForGoSignal slut: {appStatus.GetStatusTekstmsg}");
            }
            else
            {
                Trace.LogInformation($"RunOrWaitForGoSignal slut: {appStatus.GetStatusTekstmsg}");
            }
        }
    }
}
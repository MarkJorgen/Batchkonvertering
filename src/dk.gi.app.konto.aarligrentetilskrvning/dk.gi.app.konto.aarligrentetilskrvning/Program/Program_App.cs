...
namespace dk.gi.app.konto.aarligrentetilskrvning
{
    ...
    public partial class GIConsoleApp
    {
        ...
        internal AppStatus Start()
        {
            ...
            // Husk at fjerne kommentarlinjer i funktionen SendEmail hvis app skal sende fejl når afvikling ikke går godt
            try
            {
                // ****************************************
                ...
                if (this.OpretCrmConnection() == true)   // Opret forbindelse til CRM, dette kaster exception hvis der ikke kunne forbindes, og det fanges i funktion og false retuners hertil
                {
                    ...
                    //{
                        /// Din kode herunder... erstat myRequest med navn på dit request og myResponse med Generisk response eller dit eget response

                        // Tjek for valgfrie paramtere med navn ????
                        //if (appConfig.ContainsAll(new string[] { "RegnskabStartMaaned", "AntalPosterBegraesning", "UdtraeksType", "KundeId", "EmneKode", "SagsType", "SagsEmne", "OpgaveEmne", "Bruger", "DokumentNavn", "TilladSendTilDigitalPost", "WebserviceUrl", "Certificat" }))
                        //{
                        //    // Udfør job, men vi vil ikke have for mange versioner af denne applikation kørende samtidigt, fordi .......
                        result.SetStatus = RunOrWaitForGoSignal(this.CallBackFunction, "dk.gi.app.konto.aarligrentetilskrvning", false);
                        //}
                        //else
                        //{
                        //    Trace.LogError("Parametre er ikke udfyldt");
                        //    result.SetStatus = AppStatus.StateCode.AppRequiredEkstraParamMissing; // Parameter ikke udfyldt
                        //}
                    //}
                }
            ...
            if (result.statecode != AppStatus.StateCode.OK)
            {
                ...
                if (appConfig.ValidateEmailConfigurationOgEmailModtager() == true)
                {
                    ...
                    this.SendEmail((int)result.statecode);
                }
            }
            else
            {

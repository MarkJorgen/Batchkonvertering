...
{
    internal static class Ejendom
    {
        internal static void Anonymisering(CrmContext context, ref AppStatus result)
        {
            context.Trace.LogInformation("Ejendom.anonymisering start");
            try
            {
                // Opret søgning som finder alle vedligehold som opfylder kriterier
                FindOgAnonymiserEjendomRequest req = new FindOgAnonymiserEjendomRequest(context)
                {
                    //AntalAarUnChanged = 1
                };
                // Udfør query og få et resultat af de behandlede ejendomme retur
                FindOgAnonymiserEjendomResponse resp = req.Execute<FindOgAnonymiserEjendomResponse>();
                if (resp.Status.IsOK() == false)
                {
                    context.Trace.LogError("Der opstod en fejl undervejs i Anonymisering af Ejendom.");
            ...
            context.Trace.LogInformation("Ejendom.anonymisering Slut");
        }
    }
}

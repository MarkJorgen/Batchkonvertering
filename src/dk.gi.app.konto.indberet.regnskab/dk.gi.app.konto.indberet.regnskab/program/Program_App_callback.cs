    ...
    public partial class GIConsoleApp
    {
        ...
        private AppStatus CallBackFunction()
        {
            ...
            if (appConfig.Mode.ToUpper() == "BATCH")
            {
                ...
                IndberetRegnskabRequest indberetRegnskabRequest = new IndberetRegnskabRequest(crmcontext)
                {
                };
                IndberetRegnskabResponse indberetRegnskabResponse = indberetRegnskabRequest.Execute<IndberetRegnskabResponse>();

                if (indberetRegnskabResponse.Status.IsOK() == false)
                {
